...
class PrivateKey(object):
    ...
    def get_pkey_obj(self):
        ...
        password = utils.to_bytes(self.password) if self.password else None
...
class MixinHandler(object):
    ...
    def check_origin(self, origin):
        ...
        if self.origin_policy == 'same':
            ...
            return utils.is_same_primary_domain(netloc.rsplit(':', 1)[0],
                                          host.rsplit(':', 1)[0])
        else:
    ...
    def is_forbidden(self, context, hostname):
        ...
        if context._orig_protocol == 'http':
            if redirecting and not utils.is_ip_hostname(hostname):
                ip_address = utils.to_ip_address(ip)
    ...
    def get_real_client_addr(self):
        ...
        port = utils.to_int(port)
        if port is None or not utils.is_valid_port(port):
            # fake port
            port = 65535

        return (ip, port)

class IndexHandler(MixinHandler,View):

    def __init__(self, policy, host_keys_settings):
        self.policy = policy
    ...
    def get_hostname(self):
        ...
        if not (utils.is_valid_hostname(value) or utils.is_valid_ip_address(value)):
    ...
    def parse_encoding(self, data):
        try:
            encoding = utils.to_str(data.strip(), 'ascii')
        except UnicodeDecodeError:
            return

        if utils.is_valid_encoding(encoding):
            return encoding
...
class ConnectRemoteView(View):
    ...
    def post(self,request):
        ...
        settings.check_encoding_setting(options.encoding)
        loop = tornado.ioloop.IOLoop.current()
        host_keys_settings = settings.get_host_keys_settings(options)
        policy = settings.get_policy_setting(options, host_keys_settings)
        IndexHandler()
